...
export async function runFetchJobsCycle(now = Date.now()) {
  ...
  if (state.lastCycleStartedAt && now - state.lastCycleStartedAt < FETCH_DEBOUNCE_MS) {
    await safeAppendLog("debug", "Skipped fetch cycle because another cycle started recently.");
    return;
  }
  ...
  if (!state.enabled) {
    ...
    await safeAppendLog("info", "Skipped fetch cycle because notifications are disabled.");
    return;
  }
